...
namespace LEDCountDown
{
    public partial class Form1
    {
        ...
        {
            int iconSize = 256;
            var colors = CLOCK_THEMES[0]; // 典雅白陶瓷
            ...
            using (var g = Graphics.FromImage(bmp))
            {
                ...
                float r = iconSize / 2f - 4;

                // 底部光晕
                using (var glowPen = new Pen(colors.Glow, 16))
                    g.DrawEllipse(glowPen, cx - r - 4, cy - r - 4, (r + 4) * 2, (r + 4) * 2);

                // 外环阴影
                Color outlineOpaque = ToOpaque(colors.Outline);
                using (var shBrush = new SolidBrush(Darken(outlineOpaque, 0.3f)))
                    g.FillEllipse(shBrush, cx - r - 2, cy - r - 2, (r + 2) * 2, (r + 2) * 2);

                // 外环主体
                using (var ringPath = new GraphicsPath())
                {
                    ringPath.AddEllipse(cx - r, cy - r, r * 2, r * 2);
                    using (var ringBrush = new PathGradientBrush(ringPath))
                    {
                        ringBrush.CenterColor = Lighten(outlineOpaque, 0.3f);
                        ringBrush.SurroundColors = new Color[] { Darken(outlineOpaque, 0.4f) };
                        g.FillEllipse(ringBrush, cx - r, cy - r, r * 2, r * 2);
                    }
                }
                ...
                using (var facePath = new GraphicsPath())
                {
                    facePath.AddEllipse(cx - faceR, cy - faceR, faceR * 2, faceR * 2);
                    Color faceCenter = Lighten(colors.Face, 0.15f);
                    Color faceEdge = Darken(colors.Face, 0.10f);
                    using (var faceBrush = new PathGradientBrush(facePath))
                    {
                        faceBrush.CenterColor = faceCenter;
                ...
                for (int i = 0; i < 60; i++)
                {
                    ...
                    if (i % 15 == 0) { tickLen = faceR * 0.13f; tickWid = 4; tickColor = colors.TickMajor; }
                    else if (i % 5 == 0) { tickLen = faceR * 0.09f; tickWid = 3; tickColor = colors.TickMajor; }
                    else { tickLen = faceR * 0.055f; tickWid = 2; tickColor = colors.TickMinor; }

                    float ox = cx + (float)(faceR * Math.Cos(rad));
                    float oy = cy + (float)(faceR * Math.Sin(rad));
                    float ix = cx + (float)((faceR - tickLen) * Math.Cos(rad));
                    float iy = cy + (float)((faceR - tickLen) * Math.Sin(rad));

                    if (i % 15 == 0)
                    {
                        using (var shPen = new Pen(Color.FromArgb(50, 0, 0, 0), tickWid + 1))
                            g.DrawLine(shPen, ix + 1.5f, iy + 1.5f, ox + 1.5f, oy + 1.5f);
                    }
                ...
                // 4 个数字
                using (var numFont = new Font("Arial", 28, FontStyle.Regular))
                {
                    int[] nums = { 12, 3, 6, 9 };
                    float numR = faceR * 0.72f;
                    for (int ni = 0; ni < 4; ni++)
                    {
                        int hi = nums[ni] == 12 ? 0 : nums[ni];
                        double a = hi * 30 - 90;
                        double rad = a * Math.PI / 180;
                        float nx = cx + (float)(numR * Math.Cos(rad));
                        float ny = cy + (float)(numR * Math.Sin(rad));
                        string num = nums[ni].ToString();
                        var sz = g.MeasureString(num, numFont);
                        using (var brush = new SolidBrush(colors.NumColor))
                            g.DrawString(num, numFont, brush,
                                nx - sz.Width / 2, ny - sz.Height / 2);
                    }
                }

                // 指针（10:10 经典时间）
                double iconHour = 10 + 10.0 / 60;
                double iconMin = 10;
                DrawTaperedHand(g, cx, cy, (float)(iconHour * 30 - 90),
                    faceR * 0.38f, faceR * 0.09f, 2, colors.HourHand);
                DrawTaperedHand(g, cx, cy, (float)(iconMin * 6 - 90),
                    faceR * 0.58f, faceR * 0.05f, 1.5f, colors.MinHand);
                DrawSecondHand(g, cx, cy,
                    (float)(35 * 6 - 90), faceR * 0.70f, colors.SecHand, false);

                // 中心轴
                using (var cp = new GraphicsPath())
                {
                    cp.AddEllipse(cx - 10, cy - 10, 20, 20);
                    using (var cb = new PathGradientBrush(cp))
                    {
                        cb.CenterColor = Lighten(colors.GlowCenter, 0.3f);
                        cb.SurroundColors = new Color[] { Darken(colors.GlowCenter, 0.25f) };
                ...
                using (var hl = new SolidBrush(Color.FromArgb(180, 255, 255, 255)))
                    g.FillEllipse(hl, cx - 3, cy - 3, 6, 6);

                // 玻璃反光
                using (var refPath = new GraphicsPath())
                {
                    float rw = faceR * 1.3f;
                    float rh = faceR * 0.35f;
                    refPath.AddEllipse(cx - rw / 2, cy - faceR * 0.78f, rw, rh);
                    using (var refBrush = new PathGradientBrush(refPath))
                    {
                        refBrush.CenterColor = Color.FromArgb(45, 255, 255, 255);
                        refBrush.SurroundColors = new Color[] { Color.FromArgb(0, 255, 255, 255) };
                        refBrush.FocusScales = new PointF(0.1f, 0.1f);
                ...
                // ── 保存为 .ico ──
                SaveAsIcon(bmp, outputPath);
            }
        }

        /// <summary>将 Bitmap 保存为 .ico 文件（用 PNG 压缩）</summary>
        private static void SaveAsIcon(Bitmap bmp, string path)
        {
            ...
            using (var bw = new BinaryWriter(fs))
            {
                ...
                int sz = bmp.Width >= 256 ? (byte)0 : (byte)bmp.Width;
